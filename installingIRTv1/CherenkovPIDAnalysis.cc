      ...
      )
  {
    ...
    for(const auto& cherenkov_pid : cherenkov_pids) {
      ...
      */
      ...
      if(m_rad_name =="Aerogel") mc_rindex = 1.0256;
	...
	if(abs(theta-theta_exp)*1e3<=10){
          theta_rec += theta;
          nph ++;
	}
      }	
      theta_rec /= nph;//cherenkov_pid.getNpe();
      ...
      // fill PID histograms
      m_npe_dist->Fill(  nph );//cherenkov_pid.getNpe());
      m_npe_vs_p->Fill(   charged_particle_momentum,nph);// cherenkov_pid.getNpe());
      m_npe_vs_eta->Fill( charged_particle_eta, nph);//     cherenkov_pid.getNpe());
